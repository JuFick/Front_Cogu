...
export default function Avaliacoes() {
  ...
  useEffect(() => {
    async function getAvalia() {
      const response = await fetch("http://localhost:3000/avaliacoes/cogumelo/" + params.cogumelo_id)
    ...
    async function getCogumelo() {
      const response = await fetch("http://localhost:3000/cogumelos/" + params.cogumelo_id)
  ...
  if (isLoading) {
    return (
      <div>
        <h2>Listagem das Avaliações do Cogumelo</h2>
  ...
  return (
    <div className="max-w-screen-xl flex justify-around mt-2 mx-auto p-4 border border-gray-200 rounded-lg shadow">
      <div>
        <p className="mt-2 font-bold text-xl">cogumelo: {cogumelo.nome} </p>
        <img src={cogumelo.foto} alt="Capa" width={200} />
