...
export default function Avaliar() {
  ...
  const [cogumelo, setCogumelo] = useState({})
  ...
  useEffect(() => {
    async function getCogumelo() {
      const response = await fetch("http://localhost:3000/cogumelos/" + params.id)
      const dado = await response.json()
      // console.log(dado)
      setCogumelo({
        id: dado.id,
        nome: dado.nome,
        qnt_disp: dado.qnt_disp,
        preco: dado.preco,
        foto: dado.foto,
        beneficios: dado.beneficios,
        total: dado.total,
        num: dado.num
      })
    }
  ...
  async function enviaComentario(data) {
    const avaliacao = { ...data, cliente_id: idClienteLogado, cogumelo_id: cogumelo.id }

    const avalia = await fetch("http://localhost:3000/avaliacoes",
      {
        method: "POST",
  ...
  return (
    <div className="max-w-screen-xl flex flex-wrap justify-around mt-2 mx-auto p-4 border border-gray-200 rounded-lg shadow">
      <div className="max-w-sm p-6">
        <img src={cogumelo.foto} alt="cogumelo" width={500} className="mx-auto" />
        ...
        <div className="flex justify-between">
          ...
          <p className="mt-1">{cogumelo.num} avaliações</p>
        </div>
      </div>
      <div>
        <form onSubmit={handleSubmit(enviaComentario)}>
          <h3 className="font-bold text-2xl mt-4">Cadastre seu comentário sobre este cogumelo</h3>
          <hr />
          <div className="my-4">
